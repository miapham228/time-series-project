...
The notation NNAR(*p*, *k*) indicates there are *p* lagged inputs and *k* nodes in the hidden layer. For example, a NNAR(8,4) model is a neural network with the last eight observations (yt−1,yt−2,…,yt−8) used as inputs for forecasting the output *yt*, and has four neurons in the hidden layer.
...
With seasonal data, we can also add the last observed values from the same season as inputs. NNAR(*p*, *P*, *k*)m model has inputs (yt−1,yt−2,…,yt−p,yt−m,yt−2m,…,yt−Pm) and *k* neurons in the hidden layer. 
...
```{r}
nnar_model <- nnetar(train$`Local Government Administration`)
print(nnar_model)
```
...
From the output, `Model:  NNAR(5,3)` indicates that there are 5 lagged inputs used as input features for the neural network, and there are 3 nodes (neurons) in the hidden layer of the neural network.

The numbers (5-3-1) represent the number of neurons in each layer of the network: 5 lagged input, 3 hidden neurons, and 1 output neuron that produce the model's prediction.
